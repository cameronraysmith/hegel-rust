...
fn test_sampled_from_returns_element_from_list() {
    hegel::hegel(|| {
        let options = hegel::draw(&gen::vecs(gen::integers::<i32>()));
        let value = hegel::draw(&gen::sampled_from(options.clone()));
...
fn test_sampled_from_strings() {
    hegel::hegel(|| {
        let options = hegel::draw(&gen::vecs(gen::text()));
        let value = hegel::draw(&gen::sampled_from(options.clone()));
        assert!(options.contains(&value));
    });
}
...
fn test_optional_respects_inner_generator_bounds() {
    hegel::hegel(|| {
        let value: Option<i32> =
            hegel::draw(&gen::optional(gen::integers().with_min(10).with_max(20)));
        if let Some(n) = value {
...
fn test_one_of_many() {
    hegel::hegel(|| {
        ...
        let value: i32 = hegel::draw(&gen::one_of(generators));
...
fn test_optional_mapped() {
    hegel::hegel(|| {
        let value: Option<String> = hegel::draw(&gen::optional(
            gen::integers::<i32>()
                .with_min(0)
                .with_max(100)
